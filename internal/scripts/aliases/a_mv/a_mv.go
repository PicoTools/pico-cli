...
func FrontendAntMv(args ...object.Object) (object.Object, error) {
	if len(args) != 3 {
	...
	if err := BackendAntMv(uint32(id.GetValue().(int64)), src.GetValue().(string), dst.GetValue().(string)); err != nil {
		return nil, err
	}
	return object.NewNull(), nil
}

func BackendAntMv(id uint32, src, dst string) error {
	cap := shared.CapMv
